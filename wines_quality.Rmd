...
```{r}
lr_prob <- predict(lr_fit, dtest, type="response")
lr_pred <- ifelse(lr_prob > 0.355,"low","high")
cm <- confusionMatrix(as.factor(lr_pred),
                      dtest$quality,
                      positive = "low"
                      )
cm
...
```{r}
ggpairs(wwines, columns = c(2, 4, 6, 8, 
9, 10, 11), ggplot2::aes(colour=quality))
```


## DETECTING OUTLIERS
```{r, warning=FALSE}
influenceIndexPlot(lr_fit, id.n=10)
a<-dffits(lr_fit)
threshold<-(2*(11+1)/sqrt(4898-11-1))
which(abs(a)>threshold)
dtrain["outlier"]<-ifelse(rownames(dtrain) %in% c(2783,3902,4746,1971,2732,3323,4481,3737),0,1)
...
```{r}
dtrain$outlier<- NULL
outlier <- c(2783,3902,4746,1971,2732,3323,4481,3737)
dtrain_outlierfree <- dtrain[-outlier,]
