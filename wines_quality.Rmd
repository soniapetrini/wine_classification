...
Now we build a new model which will have outlier free data as training data.
```{r}
...
```
We can notice that coefficients don't change too much when removing outliers. The only one which shifts a little bit more is the coefficient relative to the intercept, which has a -0.11 change. It could be that the outlier we removed affected the intercept coefficient estimate the most.
...
## Performance --> keep it? it is the same than before
```{r}
task_train = TaskClassif$new(id = "wines_outlierfree",  dtrain_of[,-8], target = "quality", positive = "high")
...
## Performance comparison--> keep it? it is the same than before
