...
- Intercept 99.97%: this value corresponds to the increase in the odds of having a high quality wine when all the explanatory variables are set at 0.
- volatile.acidity: a unitary increase in volatile acidity will lead to a 99.87% decrease in the odds of having a high quality wine.
- residual.sugar: a unitary increase in volatile acidity will lead to a 7.44% increase in the odds of having a high quality wine.
- free.sulfur.dioxide: a unitary increase in volatile acidity will lead to a 1.36% increase in the odds of having a high quality wine.
- alcohol: a unitary increase in volatile acidity will lead to a 186.5% increase in the odds of having a high quality wine.

We find these variable which are significant at 95% confidence level:
- fixed.acidity: a unitary increase in volatile acidity will lead to a 16.68% decrease in the odds of having a high quality wine.
- sulphates: a unitary increase in volatile acidity will lead to a 283.46% increase in the odds of having a high quality wine.

We find this variable which is significant at 90% confidence level:
- total.sulfur.dioxide: a unitary increase in volatile acidity will lead to a 0.32% decrease in the odds of having a high quality wine.
...
```
The unrestricted model performs slightly better than the restricted one, but the price to pay to get 0.001 more accuracy is to include 5 more explanatory variables. To be parsimonious, we can use the restricted model anyway because it is simplier and it has all the necessary variables to reach an acceptable performance.

## Detecting outliers using restricted model 
```{r, warning=FALSE}
#dffits threshold
threshold<-(2*(ncol(wwines.res[train_indices,])+1-1))/(sqrt(nrow(wwines.res[train_indices,])-ncol(wwines.res[train_indices,])-1-1))  
d <- dffits(lr_fit_res)
#cook's distance
...
wwines[train_indices,]["outlier"] <- ifelse(rownames(wwines.res[train_indices,]) %in% outliers$.rownames,0,1)
...
lr_fit_outlierfree <- glm(quality ~., data =  wwines.res[no_outliers_train_indices,],
                          family=binomial(link='logit'))
